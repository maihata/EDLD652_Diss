...
```{r}
#LINA: DOESN'T RUN CORRECTLY --- SO I NEED THE ADJUSTED STANDARDIZED RESIDUALS 
...
```{r}
residuals_df <- chisq.test(race_matrix, correct = FALSE)$stdres
```

## Trying to get ADJUSTED standardized residuals
...
```{r}
corrplot(chi_results$residuals, 
         is.cor = FALSE, 
         tl.cex = 0.7)

# corrplot(chi_results$residuals, is.cor = FALSE) 
```
...
colnames(chi_results$residuals)
```

```{r}
colnames(chi_results$residuals) <- c(
  "Withdrawal", 
  "Disqualification", 
  "Moved Out", 
  "Part B Eligible", 
...
```{r}
rownames(chi_results$residuals) <- c(
...
```{r}
#| fig-align: center
chi_results$residuals <- chi_results$residuals[, order(colnames(chi_results$residuals))]
```
...
# residuals_df <- as.matrix(chi_results$residuals)#
# col_order <- order(colnames(residuals_df))
# row_order <- order(rownames(residuals_df))
...
### IGNORE THESE CHUNKS — I DON'T THINK I NEED THIS There used to be a Var1 and Var2 now there's only Var1. WHERE DID I MAKE residuals_matrix
...
# rownames(residuals_df) 
```

it's NULL so I need to assign names first it says

```{r}
# residual_matrix <- as.matrix(chi_results$residuals)
```
...
```{r}
# write.csv(residuals_df,         file = "/Users/hata/Desktop/EDLD652_Diss/Data/residuals_df.csv",   row.names = FALSE)
```
