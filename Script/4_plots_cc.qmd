...
```{r}
# Load ggsci for the Okabe-Ito color palette
library(ggsci)
...
ggplot(childcount1920USOR_long, 
  ...
  geom_bar(stat = "identity", 
           position = position_dodge(width = 0.7)) +  
  scale_fill_aaas() +  # Use Okabe-Ito colors directly
  scale_x_continuous(
    labels = percent_format(scale = 1),   labs(
  ...
  geom_text(aes(
    label = scales::percent(percent / 100, accuracy = 0.1)), 
            ...
            color = "black", 
            size = 4, 
            family = "Times New Roman"  # Keep Times New Roman for labels
  ) +  
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "bottom",
    text = element_text(family = "Times New Roman")  # Set Times New Roman globally
  )
```
