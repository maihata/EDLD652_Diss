...
```{r}
race_us_2 <- read.csv("/Users/maiko/Desktop/EDLD652_Diss/Data/race_us_2.csv")
```
...
write.csv(
  ...
  file = "/Users/maiko/Desktop/EDLD652_Diss/Data/odds_notdetermined_by_race.csv",
  row.names = FALSE
)
```
...
race_us_2_clean2 <- read.csv("/Users/maiko/Desktop/EDLD652_Diss/Data/race_us_2.csv") 
...
```{r}
summary_df_not_determined <- summary_df_not_determined[, 
  c("Race", "Odds Ratio", "P Value", "Lower", "Upper")]
...
summary_df_not_determined <- summary_df_not_determined %>%
  mutate(Race = trimws(as.character(Race))) %>%              # normalize
  filter(!Race %in% c("Race", "National Average"))           # drop header-like + national

```

```{r}
summary_df_not_determined
```

```{r}
write.csv(
  summary_df_not_determined, 
  "/Users/maiko/Desktop/EDLD652_Diss/Data/summary_df_odds_not_determined_national.csv"
)
```
...
write.csv(
  summary_df_h,
  file = "/Users/maiko/Desktop/EDLD652_Diss/Data/cohens_h_notdetermined_by_race.csv",
  row.names = FALSE
)
...
write.csv(
  ...
  file = "/Users/maiko/Desktop/EDLD652_Diss/Data/odds_notdetermined_plot_by_race.csv",
  row.names = FALSE
)
