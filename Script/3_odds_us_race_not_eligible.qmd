...
```{r}
tail(summary_df_odds_noteligible)
unique(summary_df_odds_noteligible$Race)
...
summary_df_odds_noteligible <- summary_df_odds_noteligible %>%
  ...
  filter(!Race %in% c("Race", "National Average"))      # drop header-like row + national

```

```{r}
write.csv(summary_df_odds_noteligible, 
          "/Users/maiko/Desktop/EDLD652_Diss/Data/summary_df_odds_not_eligible_national.csv",
          row.names = FALSE)
```
...
write.csv(
  summary_df_h,
  file = "/Users/maiko/Desktop/EDLD652_Diss/Data/cohens_h_noteligible_by_race.csv",
  row.names = FALSE
)
```
