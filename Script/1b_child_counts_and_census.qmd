...
ei_counts <- stdres_matrix_cc_census_noHis["EI Population", ]
pop_counts <- stdres_matrix_cc_census_noHis["General Population", ]
...
or_df <- data.frame(
  ...
  EI_Rate = round(group_rate, 4),
  OR_vs_PopAvg = round(or_vs_pop, 2),
  row.names = names(ei_counts)
)

# Optional: Add commas to the counts
or_df$EI_Count <- format(or_df$EI_Count, big.mark = ",")
or_df$Pop_Count <- format(or_df$Pop_Count, big.mark = ",")

# Step 6: Display as a nicely formatted table
library(knitr)
library(kableExtra)
...
or_df$Group <- rownames(or_df)
rownames(or_df) <- NULL

# Step 2: Create a reference row for the national average
ref_row <- data.frame(
  EI_Count = format(total_ei, big.mark = ","),
  Pop_Count = format(total_pop, big.mark = ","),
  EI_Rate = sprintf("%.4f", overall_rate),
  OR_vs_PopAvg = 1.00,
  Group = "National Average"
)

# Step 3: Append the reference row
or_df <- rbind(or_df, ref_row)

# Step 4: Reorder factor levels so "National Average" comes last
or_df$Group <- factor(or_df$Group, levels = c(
  setdiff(sort
